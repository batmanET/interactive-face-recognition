"""
Created on Thu Jul 9 14:13:08 2020
...
class HighAnglePersonDetector(Module):
    class Result:
        OUTPUT_SIZE = 7
    ...
    def __init__(self, model, confidence_threshold=0.5, roi_scale_factor=1):
        super(HighAnglePersonDetector, self).__init__(model)
    ...
    def enqueue(self, input):
        return super(HighAnglePersonDetector, self).enqueue({self.input_blob: input})
    ...
    def get_roi_proposals(self, frame):
        ...
        for output in outputs[0][0]:
            result = HighAnglePersonDetector.Result(output)
